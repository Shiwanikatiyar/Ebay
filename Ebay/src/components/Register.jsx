import React from 'react'
import {
  Box, Flex, Image, Link, Text, Alert,
  AlertIcon, AlertDescription,
  Container,
  Heading, Input,
  Stack,
  Button,
  HStack,
} from '@chakra-ui/react'
import { IoCheckmarkDoneCircle } from "react-icons/io5";
import { RxCrossCircled } from "react-icons/rx";
import { useNavigate } from 'react-router-dom'
import {useContext, useState } from 'react'
import { AuthContext } from "../Context/AuthContext";
import { useToast } from "@chakra-ui/react";

function Register() {
  const navigate = useNavigate()
  const moveSign = () => {
    navigate("/signin")
  }

  const [form, setForm] = useState({
    firstName: '',
    lastName: '',
    email: '',
    password: '',
  });
 

  const { setIsAuth } = useContext(AuthContext);



  const handleChange = (e) => {
    const { name, value } = e.target;
    setForm({
      ...form,
      [name]: value
    });
  };


  const toast = useToast();


  const handleSubmit = (e) => {
    e.preventDefault();

    const { email, password } = form;

    if (email.trim() && password.trim()) {
      localStorage.setItem("userData", JSON.stringify(form));
      setForm({ firstName: "", lastName: "", email: "", password: "" });


      toast({
        title: "Account created successfully, now ready to login",
        status: "success",
        duration: 3000,
        isClosable: true,
      
      });

      setIsAuth(true);

      navigate('/signin')

    }

  else {
      toast({
        title: "Please Enter Valid Credentails",
        status: "error",
        duration: 3000,
        isClosable: true,
        
      });
    }

    console.log('userData',form);

  }
  return (
    <div>
      <Flex  justifyContent="space-around" flexWrap={{ base: 'wrap', lg: 'nowrap' }}>
        <Box boxSize={'50%'} w={{ base: '100%', lg: '50%' }} mb={{ base: 8, lg: 0 }}>
          <Image
            maxW={"120px"}
            marginLeft={'2em'}
            py={'1em'}
            src= 'data:image/png;base64,   '
            alt='EbayLogo' />

          <Image src='https://ir.ebaystatic.com/cr/v/c01/buyer_dweb_individual.jpg'
            borderRadius={'1em'}
          />

        </Box>

        <Box boxSize={'50%'} w={{ base: '100%', lg: '50%' }}>
          <Stack spacing={8}>

            <Text marginTop={'2em'} paddingRight={'1em'} style={{ color: '#919191', textAlign: 'right' }}>Already have an account? <Link onClick={moveSign} style={{ color: 'black' }}>Sign in</Link>
            </Text>
            <Container>
              <Box marginBottom={'2em'}>
                <Alert status='error' background={'#3665F3'} style={{ color: 'white' }}>
                  <AlertIcon style={{ color: 'white' }} marginBottom={'6em'} />
                  <AlertDescription> To buy and sell on www.ebay.com or other eBay sites internationally,
                    existing users can login using their credentials or new users can
                    register an eBay account on ebay.in.
                    Kindly note you can no longer buy or sell on eBay.in</AlertDescription>
                </Alert>
              </Box>
              <Box >
                <Stack spacing={6}>
                  <Heading>Create an account</Heading>
                  <Text borderRadius={'3em'} border={'1px solid'} h={'3em'}
                    style={{ textAlign: 'center', paddingTop: '0.5em', background: 'black', color: 'white' }} >
                    Personal Details</Text>
                  <HStack spacing={4} >
                    <Input placeholder='First name'
                    required
                      value={form.firstName}
                      name='firstName'
                      type='name'
                      onChange={handleChange}
                      size='lg' />
                    <Input placeholder='Last name'
                    required
                      value={form.lastName}
                      name='lastName'
                      type='name'
                      onChange={handleChange}
                      size='lg' />
                  </HStack>
                  <Input placeholder='Email'
                  required
                    value={form.email}
                    type='email'
                    name='email'
                    onChange={handleChange}
                    size='lg' />
                  <Input placeholder='Password'
                  required
                    value={form.password}
                    name='password'
                    onChange={handleChange}
                    type='password' size='lg' />


                  <Text style={{ color: 'grey' }} >By selecting Create personal account,
                    you agree to our <Link style={{ textDecoration: 'underline' }}>User Agreement</Link>  and acknowledge
                    reading our <Link style={{ textDecoration: 'underline' }}>User Privacy Notice.</Link></Text>

                  <Button borderRadius={'5em'} border={'1px solid'} type='submit' onClick={handleSubmit}>Create Account</Button>
                </Stack>
              </Box>

            </Container>
          </Stack>
        </Box>
      </Flex>
      <Text style={{ color: 'grey', marginTop: '2em' }}>Copyright © 1995-2024 eBay Inc. All Rights Reserved.
        Accessibility, User Agreement, Privacy, Payments Terms of Use,
        Cookies, CA Privacy Notice, Your Privacy Choices and AdChoice.</Text>
    </div>
  )
}

export default Register


